...
for lamb in lambda_val:
    lamb_reward = []
    for r in range(10):
        fileName = "results/" + "sarsa1_replace_" + str(lamb) + "_rs" + str(r) + ".txt"
    ...
    SR_1.append(1.0 * sum(lamb_reward)/len(lamb_reward))
...
for lamb in lambda_val:
    lamb_reward = []
    for r in range(10):
        fileName = "results/" + "sarsa1_accum_" + str(lamb) + "_rs" + str(r) + ".txt"
        resultFile = open(fileName)
        print resultFile.readline()
        rewards = map(int, list(resultFile.readline().split("[")[1].split("]")[0].split(", ")))
        resultFile.close()
...
plt.plot(lambda_val, SR_1, 'r--v', label="Replacing")
...
plt.savefig("sarsa1.png", bbox_inches='tight')
# plt.show()
plt.close()